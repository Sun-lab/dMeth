---
title: "Preare gene expression data for TCGA LUAD samples"
output:
  html_document:
    df_print: paged
---
...
```{r}
data_dir = "TCGA_data/LUAD/gdc_download_20201221_234745.015148/"
info_dir = "TCGA_data/LUAD"
...
saveRDS(count.df, file="TCGA_data/TCGA_LUAD_raw_counts.rds")
```
...
LUAD_TPM   = calculate_TPM(count.df, geneLength) 
dim(LUAD_TPM)
LUAD_TPM[1:5,1:5]
```
...
table(gene.id == rownames(LUAD_TPM))

# Update LUAD_TPM to  genes with 1-1 mapping between 
# their ensembl ID and hgnc symbols 
w2match = match(geneinfo.filtered$geneId, gene.id)
LUAD_TPM_filtered = data.frame(geneSymbol = geneinfo.filtered$hgnc_symbol) 
LUAD_TPM_filtered = cbind(LUAD_TPM_filtered, LUAD_TPM[w2match,])
dim(LUAD_TPM_filtered)
LUAD_TPM_filtered[1:5,1:5]
...
genes2use = intersect(genes2use, LUAD_TPM_filtered$geneSymbol)
length(genes2use)

match.TCGA = match(genes2use, LUAD_TPM_filtered$geneSymbol)

LUAD_TPM_filtered = LUAD_TPM_filtered[match.TCGA,]
LUAD_TPM_filtered[,-1] = signif(LUAD_TPM_filtered[,-1], 5)

dim(LUAD_TPM_filtered)
LUAD_TPM_filtered[1:2,1:5]

write.table(LUAD_TPM_filtered, file = "TCGA_data/TCGA_LUAD_TPM.txt", 
