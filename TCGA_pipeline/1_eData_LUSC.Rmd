---
title: "Preare gene expression data for TCGA LUSC samples"
output:
  html_document:
    df_print: paged
---
...
```{r}
data_dir = "TCGA_data/LUSC/gdc_download_20201221_235552.499347/"
info_dir = "TCGA_data/LUSC"
...
saveRDS(count.df, file="TCGA_data/TCGA_LUSC_raw_counts.rds")
```
...
LUSC_TPM   = calculate_TPM(count.df, geneLength) 
dim(LUSC_TPM)
LUSC_TPM[1:5,1:5]
```
...
table(gene.id == rownames(LUSC_TPM))

# Update LUSC_TPM to  genes with 1-1 mapping between 
# their ensembl ID and hgnc symbols 
w2match = match(geneinfo.filtered$geneId, gene.id)
LUSC_TPM_filtered = data.frame(geneSymbol = geneinfo.filtered$hgnc_symbol) 
LUSC_TPM_filtered = cbind(LUSC_TPM_filtered, LUSC_TPM[w2match,])
dim(LUSC_TPM_filtered)
LUSC_TPM_filtered[1:5,1:5]
...
genes2use = intersect(genes2use, LUSC_TPM_filtered$geneSymbol)
length(genes2use)

match.TCGA = match(genes2use, LUSC_TPM_filtered$geneSymbol)

LUSC_TPM_filtered = LUSC_TPM_filtered[match.TCGA,]
LUSC_TPM_filtered[,-1] = signif(LUSC_TPM_filtered[,-1], 5)

dim(LUSC_TPM_filtered)
LUSC_TPM_filtered[1:2,1:5]

write.table(LUSC_TPM_filtered, file = "TCGA_data/TCGA_LUSC_TPM.txt", 
