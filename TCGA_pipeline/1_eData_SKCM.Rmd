...
```{r}
data_dir = "TCGA_data/SKCM/gdc_download_20201012_225902.677830/"
info_dir = "TCGA_data/SKCM"

# sample information
sample_info = read.table(file.path(info_dir, "gdc_sample_sheet.2020-10-12.tsv"), 
...
sam2 = sample_info[sample_info$Case.ID %in% names(tab.case)[tab.case > 1],6:8]
...
saveRDS(count.df, file="TCGA_data/TCGA_SKCM_raw_counts.rds")
```
...
# Update SKCM_TPM to  genes with 1-1 mapping between 
...
genes2use = intersect(genes2use, SKCM_TPM_filtered$geneSymbol)
length(genes2use)

match.TCGA = match(genes2use, SKCM_TPM_filtered$geneSymbol)

SKCM_TPM_filtered = SKCM_TPM_filtered[match.TCGA,]
SKCM_TPM_filtered[,-1] = signif(SKCM_TPM_filtered[,-1], 5)

dim(SKCM_TPM_filtered)
SKCM_TPM_filtered[1:2,1:5]

write.table(SKCM_TPM_filtered, file = "TCGA_data/TCGA_SKCM_TPM.txt", 
