...
for(i in 1:length(utypes)){
  ...
  save(plist, file = sprintf('plist_%s_%s.RData',utypes[i],'LUAD'))
  dev.off()
}
...
p1 <- ggplot(tempdata,aes(x=Methods,y=Correlation)) + geom_boxplot() +
  geom_point(size = 5,aes(colour = cellType)) + theme_cowplot() + ggtitle('Correlation for LUAD')
print(p1)
save(p1,file=sprintf('Cor_%s.RData','LUAD'))
dev.off()
...
p2 <- ggplot(tempdata,aes(x=Methods,y=RMSE)) + geom_boxplot() +
  geom_point(size = 5,aes(colour = cellType)) + theme_cowplot() + ggtitle('RMSE for LUAD')
print(p2)
save(p2,file=sprintf('err_%s.RData','LUAD'))
dev.off()
