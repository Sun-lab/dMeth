...
for(c1 in cancer_types){
  ...
  df2 = as.data.frame(cbind(df1[1:7,], df1[8:14,]))
  rownames(df2) = gsub(".E", "", rownames(df2), fixed=TRUE)
  print(df2)
}
```
...
for(c1 in cancer_types){
  ...
  df1 = t(sapply(cell_types, cox_summary, ct=ct, surv_var="DSS"))
  kk  = (nrow(df1)/2)
  df2 = as.data.frame(cbind(df1[1:kk,], df1[(kk+1):nrow(df1),]))
  rownames(df2) = gsub(".E", "", rownames(df2), fixed=TRUE)
  print(df2)
}
```

Generate plots for log2 cell type fraction ratios for SKCM.
...
for(ctype1 in cell_types){
  ...
  g2 = ggsurvplot(fit1, data = ct, risk.table = TRUE, 
                  pval=TRUE, pval.method=TRUE, 
                  title=ctype1)
...
for(c1 in cancer_types){
  ...
  gga = ggarrange(gc1, gc2 + rremove("legend"), 
